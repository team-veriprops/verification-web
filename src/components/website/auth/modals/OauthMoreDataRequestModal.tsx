...
import { EmailSource, SocialLoginUserInfoDto } from '../models';

export default function OauthMoreDataRequestModal({userInfo}:{userInfo: SocialLoginUserInfoDto | null}) {
    ...
    >
      ...
      <DialogContent showCloseButton={false} className={
        ...
        <div className={cn("space-y-6", currentStep !== 'email_phone' && "hidden")}>
            <VerifyEmailPhoneForm onSetCurrentStep={setCurrentStep} email={userInfo?.email} emailSource={userInfo?.provider as unknown as EmailSource} emailOtp={userInfo?.otp} emailVerified={userInfo?.email_verified} />
